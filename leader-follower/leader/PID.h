...
class PID_c {
  public:
    ...
    float update( float demand, float measurement ) {
      ...
      // FIX: remember this error for the next call's derivative.
      // (Previously last_error was set right after error was computed, so
      // diff_error was always 0 and the D term did nothing.)
      last_error = error;

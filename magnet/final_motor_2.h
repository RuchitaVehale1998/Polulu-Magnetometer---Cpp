...
#define _MOTORS_H
# define FWD HIGH
# define REV LOW   // FIX: was HIGH (same as FWD); direction macros must differ
...
#define L_DIR 16 // This is the  pin! 
#define R_PWM 9 // This is the  pin! 
#define R_DIR 15 // This is the  pin! 
...
class Motors_c {
  ...
  public:
    ...
    void setPWM( float left_pwr, float right_pwr ) {
      ...
      // FIX: clamp magnitude to [0, MAX_PWM] so large demands can't wrap past 255.
      analogWrite( L_PWM, constrain( abs(left_pwr),  0, MAX_PWM ) );
      analogWrite( R_PWM, constrain( abs(right_pwr), 0, MAX_PWM ) );
